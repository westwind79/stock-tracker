...
const Dashboard = () => {
  ...
  const refreshData = async () => {
    try {
      setRefreshing(true);
      toast.info('Fetching fresh data from SEC EDGAR...');
      
      const response = await axios.post(`${API}/transactions/refresh`);
      toast.success(`Refreshed ${response.data.transactions_count} transactions`);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              <CardContent>
                {filteredTransactions.length === 0 ? (
                  <div className="text-center py-12" data-testid="no-transactions">
                    <p className="text-slate-500">No transactions found. Click "Refresh" to load data from SEC.</p>
                  </div>
                ) : (
        ...
        <div className="mt-12 text-center text-sm text-slate-500">
          <p>Data sourced from SEC EDGAR • Updated in real-time</p>
